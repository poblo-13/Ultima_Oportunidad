...
CREATE OR REPLACE PROCEDURE sp_procesar_facturacion(p_fecha DATE, p_valor_dolar NUMBER) IS
    CURSOR c_checkout IS
        SELECT r.id_huesped, 
               ...
               MAX(r.estadia) as estadia,
               SUM(hab.valor_habitacion) AS total_hab,
               SUM(hab.valor_minibar) AS total_minibar,
               -- REGLA PROFE: Cantidad de personas según tipo de habitación
        ...
        GROUP BY r.id_huesped, h.nom_huesped, h.appat_huesped, h.apmat_huesped;
...
BEGIN
    ...
    FOR reg IN c_checkout LOOP
        ...
        -- subtotal alojamiento + consumos + personas
        ...
        --  total = subtotal - dctos NO SUMAR TOURS
        v_total_final := v_subtotal_clp - v_desc_con_clp - v_desc_age_clp;
...
SELECT id_huesped, nombre, agencia, alojamiento, consumos, tours, subtotal_pago, descuento_consumos, descuentos_agencia, total
